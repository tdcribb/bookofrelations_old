...
function bp_core_get_directory_pages() {
	...
	if ( !empty( $page_ids ) ) {
		...
		$page_names       = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_name, post_parent, post_title FROM {$posts_table_name} WHERE ID IN ({$page_ids_sql}) AND post_status = 'publish' ", $page_id ) );
